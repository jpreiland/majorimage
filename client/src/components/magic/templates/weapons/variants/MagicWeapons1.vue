<template>
  <CollapsibleSection title="Weapon Description">
    <p>
  ...
  <CollapsibleSection title="Modified Weapon">
    <Descriptor type="ModifiedWeapon" color="#8e1fde" :key="'ModifiedWeapon-' + rerollToggle" />
  </CollapsibleSection>

  <CollapsibleSection title="Custom Weapon Names">
    <template v-for="weapon in customWeapons" :key="'Weapon-' + weapon.id + '-' + rerollToggle">
      <p>
...
const numCustomWeapons = 10

const customWeapons = ref<{ id: number }[]>([])
const rerollToggle = ref(true)

onMounted(() => {
  customWeapons.value = Array.from(
    { length: numCustomWeapons },
    (_, id) => ({ id })
  )
})
