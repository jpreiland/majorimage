  ...
  <div>
    <div class="columns list left-scroll-menu">
      <div class="column left-scroll-menu">
        <ul>
          <li class="list-item" :class="{ 'selected': this.initialized && types[i].selected }" v-for="(npcSubpage, key, i) in data.templates.npcs" @click="select(npcSubpage, i)" :key="'npcSubpage-'+key">{{ npcSubpage._displayName }}</li>
        </ul>
      </div>
