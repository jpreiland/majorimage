  ...
  <div>
    <div class="columns list left-scroll-menu">
      <div class="column left-scroll-menu">
        <ul>
          <li class="list-item" :class="{ 'selected': this.initialized && types[i].selected }" v-for="(objectSubpage, key, i) in data.templates.objects" @click="select(objectSubpage, i)" :key="'objectSubpage-'+key">{{ objectSubpage._displayName }}</li>
        </ul>
      </div>
